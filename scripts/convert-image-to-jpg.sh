#!/bin/zsh
ACTION_NAME="convert-image-to-jpg"
...
process_file() {
  ...
  local out; out="$(unique_output_path "${dir}" "${base}" "jpg")"
  require_tool "${MAGICK}" "imagemagick" || return 1
  local src; src="$(rasterize_if_svg "${input}")"
  if ! "${MAGICK}" "${src}" -background white -alpha remove -alpha off -quality 92 "${out}" >>"${LOG_FILE}" 2>&1; then
    [[ "${src}" != "${input}" ]] && rm -f "${src}"
    return 1
  fi
  [[ "${src}" != "${input}" ]] && rm -f "${src}"
